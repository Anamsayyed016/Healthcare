        ...
        >
          {items.map((service, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`relative h-full min-h-[200px] rounded-[28px] bg-linear-to-br ${service.accent ?? 'from-white to-[#F8FBFF]'} border border-white/90 shadow-[0_10px_40px_-16px_rgba(79,157,248,0.15)] group-hover:shadow-[0_24px_56px_-20px_rgba(79,157,248,0.2)] transition-all duration-500 p-7 sm:p-8 flex flex-col`}
                >
                  <div className="flex items-start justify-between mb-6">
                    <div
                      className={`w-14 h-14 rounded-2xl bg-linear-to-br ${service.iconBg ?? 'from-[#4F9DF8] to-[#38BDF8]'} flex items-center justify-center shadow-lg`}
                    >
                  ...
                  <div className="mt-6 h-1 w-12 rounded-full bg-linear-to-r from-[#4F9DF8] to-[#4ADE80] opacity-60 group-hover:w-20 transition-all duration-500" />
                </div>
              </motion.article>
            );
          })}
