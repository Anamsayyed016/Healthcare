...
export default function LeaderPortrait({ name, large, className = '', src }: LeaderPortraitProps) {
  return (
    <div
      ...
      aria-label={src ? `Portrait of ${name}` : `Portrait placeholder for ${name}`}
    >
      {src ? (
        <Image
          src={cloudinaryUrl(src, large ? 640 : 480)}
          alt={name}
          fill
          sizes={large ? '(max-width: 1024px) 100vw, 384px' : '(max-width: 640px) 100vw, 320px'}
